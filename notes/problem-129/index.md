# Problem 129

**Status:** E1 — AI cross-checked; **NOT HUMAN VERIFIED**

**Formal verification:** Conditional F1 — full-paper formalization is partial

**Version:** v1.0

**First public release:** PENDING

> **No independent human mathematical review has been performed.**

## Files

- [PDF](paper.pdf)
- [TeX source](paper.tex)
- [Lean source](Main.lean)
- [Provenance](PROVENANCE.md)
- [Formal verification record](FORMAL_VERIFICATION.md)

## AI generation

Mathematical drafts were generated with **GPT-5.6 Sol** under the supervision of:

- Chuyang Chen — 22535018@zju.edu.cn
- Chenhao Bian — bch26@mails.tsinghua.edu.cn

## Verification notice

Successful Lean verification establishes correctness of the formalized statement relative to its assumptions. It does not by itself establish that the formalized statement is exactly equivalent to the historical problem as originally formulated.

## Feedback

Readers who identify an error, know relevant prior literature, or wish to check the argument are encouraged to contact the coordinators or open an issue in this repository.

[Back to repository index](../../)

