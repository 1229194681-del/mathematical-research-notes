...
/-!
# Nikliborc Problem 128 — standalone strict source
...
for Problem 128.  It does not import any project-local Nikliborc module.
-/
...
theorem nearBall
    ...
    ∃ τ > 0, ∀ t, 0 < t → t < τ → Nonempty (NearBallResult pub a t) := by
...
/-!
# Complete formal deduction for Nikliborc Problem 128

Relative only to the public theorem interfaces in `PublishedTheory`, this file
proves every article-specific step L128.1--L128.8.  The Newtonian potential is the
actual integral from `NikliborcCore`; no polynomial-inclusion certificate is used.
-/

namespace Nikliborc128

open Set MeasureTheory
open Nikliborc

/-- Exact machine-readable counterexample to the modernized printed Problem 128. -/
structure Counterexample128 (pub : PublishedTheory) where
  domain : Set R3
  body : Set R3
  domain_open : IsOpen domain
  body_eq_closure : body = closure domain
  body_closed : IsClosed body
  body_bounded : NormBounded body
  unicoherent : UnicoherentSet body
  t : ℝ
  t_ne_zero : t ≠ 0
  potential_polynomial : ∀ x ∈ body, newtonPotential body x = p 1 t x
  nonellipsoid : ¬ IsSolidEllipsoid body

/-- Boundary extension for the *closed solid* occurring in the historical problem. -/
lemma closed_body_polynomial_potential
    (pub : PublishedTheory) {a t : ℝ} (nb : NearBallResult pub a t) :
    ∀ x ∈ closure nb.K, newtonPotential (closure nb.K) x = p a t x := by
  intro x hx
  have hvol := newtonPotential_closure_eq pub nb
  rw [hvol]
  exact potential_on_closure pub nb x hx

/-- Negative answer to Problem 128 under precisely the allowed published inputs. -/
theorem negative_answer_128 (pub : PublishedTheory) :
    Nonempty (Counterexample128 pub) := by
  have ha : (0 : ℝ) < 1 := by norm_num
  have hab : (1 : ℝ) < 5/4 := by norm_num
  have hbr : (5/4 : ℝ) < 3/2 := by norm_num
  have hrR : (3/2 : ℝ) < 2 := by norm_num
  have hM : (0 : ℝ) < 1 := by norm_num
  have hε : (0 : ℝ) < 1/8 := by norm_num
  have hεmargin : (1/8 : ℝ) < 3/2 - 1 := by norm_num
  obtain ⟨τ, hτ, hnear⟩ := nearBall pub ha hab hbr hrR hM hε hεmargin
  let t : ℝ := τ/2
  have htpos : 0 < t := by dsimp [t]; linarith
  have htlt : t < τ := by dsimp [t]; linarith
  obtain ⟨nb⟩ := hnear t htpos htlt
  let body := closure nb.K
  have hbodyC : body = nb.C := by simp [body, nb.closure_eq]
  have hbnd : NormBounded body := by simpa [hbodyC] using nb.C_bounded
  have huniC : UnicoherentSet nb.C := by
    rw [nb.C_image]
    apply pub.unicoherent_homeomorph_image nb.Ψ.homeomorph
    exact pub.closedBall_unicoherent ha
  have huni : UnicoherentSet body := by simpa [hbodyC] using huniC
  have hpot : ∀ x ∈ body, newtonPotential body x = p 1 t x := by
    intro x hx
    exact closed_body_polynomial_potential pub nb x (by simpa [body] using hx)
  have hnell : ¬ IsSolidEllipsoid body := by
    simpa [body] using nearBall_body_not_ellipsoid pub ha (ne_of_gt htpos) nb
  exact ⟨{
    domain := nb.K
    body := body
    domain_open := nb.K_open
    body_eq_closure := rfl
    body_closed := isClosed_closure
    body_bounded := hbnd
    unicoherent := huni
    t := t
    t_ne_zero := ne_of_gt htpos
    potential_polynomial := hpot
    nonellipsoid := hnell
  }⟩

end Nikliborc128

#print axioms Nikliborc128.negative_answer_128
